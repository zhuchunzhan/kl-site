...
<%@ include file="/WEB-INF/views/modules/site/include/taglib.jsp"%>
<%@ taglib prefix="sitemesh" uri="http://www.opensymphony.com/sitemesh/decorator" %>
<!DOCTYPE html>
<html>
<head>
	<title><sitemesh:title default="欢迎光临"/> - ${site.title}</title>
	<%@include file="/WEB-INF/views/modules/site/include/head.jsp" %>
        ...
        <div class="container">
          <c:choose>
   			<c:when test="${not empty site.logo}">
   				<img alt="${site.title}" src="${site.logo}" class="container" onclick="location='${ctx}'">
   			</c:when>
   			<c:otherwise><a class="brand" href="${ctx}">${site.title}</a></c:otherwise>
   		  </c:choose>
          <div class="nav-collapse">
            <ul id="main_nav" class="nav nav-pills">
            	<!--<li class="${not empty isIndex && isIndex ? 'active' : ''}"><a href="${ctx}/index-${site.id}${fns:getUrlSuffix()}"><span>首　 页</span></a></li>
				-->
				<c:forEach items="${fnc:getMainNavList(site.id)}" var="category" varStatus="status">
		    		...
		    		<li class="${requestScope.category.id eq category.id||fn:indexOf(requestScope.category.parentIds,menuCategoryId) ge 1?'active':''}">
		    			<a href="${ctx}/category-${category.id}${fns:getUrlSuffix()}"><span>${category.name}</span></a>
			    		<ul class="dropdown-menu">
						  <c:forEach items="${fnc:getCategoryList(site.id,category.id,-1,null)}" var="subCategory"><li><a href="${subCategory.url}" target="${subCategory.target}">${subCategory.name}</a></li></c:forEach>
						</ul>
		    		</li>
		    	</c:forEach>
